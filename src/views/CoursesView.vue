    ...
    <div class="container">
      <div class="page__navigation">
        ...
        <p class="page__navigation-current">
          <span>/</span> Список курсов
        </p>
      </div>

      <h1 class="page__title">Список курсов</h1>
...
import CourseCard from "@/components/Base/course/CourseCard.vue"
...
async function getCourses() {
  ...
  querySnapshot.forEach((doc) => {
    coursesArray.value.push(doc.data())
  })
}
...
function calculateRange() {
  ...
  let tmp
  for (let i = coursesArray.value.length - 1; i >= 0; i--) {
    tmp = +(coursesArray.value[i].pricelessons)
...
function filteredByTeachTime() {
  if (teachTime.value !== undefined) {
    return coursesArray.value.filter(course => course.time === teachTime.value)
  } else {
...
function filteredByTeachYear() {
  if (teachYear.value !== undefined) {
    return filteredByPrice().filter(course => course.years === teachYear.value)
  } else {
...
const pages = computed(() => {
  if (isApplied.value) {
    ...
    return Math.ceil(coursesArray.value.length / 6)
  }
})
